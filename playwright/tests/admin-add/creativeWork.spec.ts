...
import { adminEditTabbedView, assertAffiliationsAllowed } from '../../support/adminEdit';
...
test.describe('As an admin, I can', () => {
    ...
    const loadNtroTabAdminAdd = async (page: Page, collectionRowId: number, subtypeName: string) => {
        await page.locator('[data-testid=rek-ismemberof-input]').fill('a');
        await clickAutoSuggestion(page, 'rek-ismemberof', collectionRowId);

        // Choose display type
        await page.locator('[data-testid=rek-display-type-select]').click();
        await page
            .locator('[data-testid=rek-display-type-options]')
            .locator('li', { hasText: /Creative Work/ })
            .first()
            .click();

        // Choose subtype
        await page.locator('[data-testid=rek-subtype-select]').click();
        await page
            .locator('[data-testid=rek-subtype-options]')
    ...
    const assertCreateAuthor = async (page: Page, nameAsPublished: string) => {
        ...
        await page.locator('[data-testid="rek-author-input"]').fill(nameAsPublished);
    ...
    const assertUpdateAuthor = async (page: Page, index: number, nameAsPublished: string) => {
        ...
        await page.locator('[data-testid="rek-author-input"]').clear();
        await page.locator('[data-testid="rek-author-input"]').fill(nameAsPublished);
        await page.locator('[data-testid="rek-author-update-save"]').click();
    };
    ...
    const assertCanSetScaleStatement = async (page: Page, index: number, typeIndex: number, statement: string) => {
        ...
        await page.locator('[data-testid="rek-significance-select"]').click();
        ...
        await page.locator('[data-testid="rek-significance-add"]').click();
    };
    ...
    test('add a creative work', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="statement-item-0"]')).toHaveText(statement1);
        await expect(page.locator('[data-testid="statement-item-1"]')).toHaveText(statement2);
        // Change the author order
        await page.locator('[data-testid="rek-author-list-row-0-move-down"]').click();
        await assertHasAuthor(page, 0, 'Second Author');
        await assertHasAuthor(page, 1, 'Test Author');
        await expect(page.locator('[data-testid="statement-item-0"]')).toHaveText(statement2);
        await expect(page.locator('[data-testid="statement-item-1"]')).toHaveText(statement1);
        // Clear one of the statements
        await assertCanClearScaleStatement(page, 0);
        await expect(page.locator('[data-testid="statement-item-0"]')).toHaveText(/Missing/);
        ...
        await page.locator('[data-testid="rek-author-list-row-0-delete"]').click();
        await page.waitForTimeout(500);
        await page.locator('[data-testid="rek-author-delete-save"]').click();
        await assertHasAuthor(page, 0, 'Test Author');
        // Clear the statement via edit
        await page.locator('[data-testid="rek-significance-list-row-0-edit"]').click();
        // set true
        await page.locator('[data-testid="empty-significance-statement-input"]').click();
        // set false (to toggle)
        await page.locator('[data-testid="empty-significance-statement-input"]').click();
    ...
    test('delete all added creative work scale of significance and creative statements', async ({ page }) => {
        ...
        await page.locator('[data-testid="delete-all-rek-significance"]').click();
        await expect(page.locator('[data-testid="confirm-rek-significance-delete-all"]')).toHaveText(/Yes/);
        await page.locator('[data-testid="confirm-rek-significance-delete-all"]').click();
        await expect(page.locator('[data-testid="rek-significance-list"]')).toHaveText(/No records to display/);
    });
