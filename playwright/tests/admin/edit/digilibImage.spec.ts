...
test.describe('Digilib Image admin edit', () => {
    ...
    test('should render the different sections as expected', async ({ page }) => {
        ...
        await expect(bibliographicTab.locator('.AdminCard').nth(2).locator('h4')).toHaveText(/Geographic co-ordinates/);
        await expect(bibliographicTab.locator('.AdminCard').nth(2).getByTestId('rek-geographic-area')).toBeVisible();

        await expect(bibliographicTab.locator('.AdminCard').nth(3).locator('h4')).toHaveText(/Related publications/);
        ...
        for (const [index, pub] of relatedPubs.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(3).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(4).locator('h4')).toHaveText(/Period\(s\)/);
        const periods = record.fez_record_search_key_period.map(pub => pub.rek_period);
        for (const [index, pub] of periods.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(4).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(5).locator('h4')).toHaveText(/Structural system\(s\)/);
        ...
        for (const [index, pub] of structuralSystems.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(5).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(6).locator('h4')).toHaveText(/Style/);
        const styles = record.fez_record_search_key_style.map(pub => pub.rek_style);
        for (const [index, pub] of styles.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(6).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(7).locator('h4')).toHaveText(/Subcategory\(ies\)/);
        ...
        for (const [index, pub] of subcategories.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(7).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(8).locator('h4')).toHaveText(
            /Surrounding feature\(s\)/,
        );
        ...
        for (const [index, pub] of surroundingFeatures.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(8).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(9).locator('h4')).toHaveText(/Interior feature\(s\)/);
        ...
        for (const [index, pub] of interiorFeatures.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(9).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(10).locator('h4')).toHaveText(/Building material\(s\)/);
        ...
        for (const [index, pub] of buildingMaterials.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(10).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(11).locator('h4')).toHaveText(/Category\(ies\)/);
        const categories = record.fez_record_search_key_category.map(pub => pub.rek_category);
        for (const [index, pub] of categories.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(11).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(12).locator('h4')).toHaveText(/Condition\(s\)/);
        ...
        for (const [index, pub] of conditions.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(12).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(13).locator('h4')).toHaveText(/Alternative title\(s\)/);
        ...
        for (const [index, pub] of alternativeTitles.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(13).locator('p').nth(index)).toHaveText(pub);
        }

        await expect(bibliographicTab.locator('.AdminCard').nth(14).locator('h4')).toHaveText(
        ...
        for (const [index, pub] of architecturalFeatures.entries()) {
            await expect(bibliographicTab.locator('.AdminCard').nth(14).locator('p').nth(index)).toHaveText(pub);
        }
        ...
        await expect(authorDetailsTab.locator('.AdminCard').nth(0).locator('h4')).toHaveText(/Architects/);
        ...
        await expect(authorDetailsTab.locator('.AdminCard').nth(1).locator('h4')).toHaveText(/Photographers/);
        ...
        await expect(authorDetailsTab.locator('.AdminCard').nth(2).locator('h4')).toHaveText(/Other contributors/);
