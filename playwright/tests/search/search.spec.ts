...
test.describe('Search', () => {
    ...
    async function assertFirstRowItemCount(page: Page, expectedCount: number): Promise<void> {
        const items = page.locator('li[data-testid^=image-gallery-item-]');
        await expect(items).toHaveCount(8);

        const top = await items.nth(0).evaluate(el => el.offsetTop);
        const sameRowItems = await items.evaluateAll(
            (elements, top) => elements.filter(el => el.offsetTop === top),
            top,
        );
        expect(sameRowItems).toHaveLength(expectedCount);
    }
