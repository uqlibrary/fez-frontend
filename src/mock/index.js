...
mock
    .onGet(new RegExp(escapeRegExp(routes.CURRENT_ACCOUNT_API().apiUrl).replace(/\?.*$/, '.*'))).reply(config => {
    // mock account response
    if (user === 'anon') return [403, {}];
    if (mockData.accounts[user]) return [200, mockData.accounts[user]];
    return [404, {}];
})
    .onGet(new RegExp(escapeRegExp(routes.AUTHOR_DETAILS_API({userId: user}).apiUrl))).reply(config => {
    ...
    .onGet(new RegExp(escapeRegExp(routes.CURRENT_AUTHOR_API().apiUrl))).reply(config => {
    // mock current author details from fez
    if (user === 'anon') return [403, {}];
    if (mockData.currentAuthor[user]) return [200, mockData.currentAuthor[user]];
    return [404, {}];
})
    .onGet(new RegExp(escapeRegExp(routes.ACADEMIC_STATS_PUBLICATION_YEARS_API({userId: user}).apiUrl)))
    .reply(200, mockData.publicationYearsBig)
    .onGet(new RegExp(escapeRegExp(routes.ACADEMIC_STATS_PUBLICATION_HINDEX_API({userId: user}).apiUrl)))
    .reply(200, mockData.hindexResponse)
    .onGet(new RegExp(escapeRegExp(routes.ACADEMIC_STATS_PUBLICATION_STATS_API({userId: user}).apiUrl)))
    .reply(200, mockData.publicationStats)
    .onGet(new RegExp(escapeRegExp(routes.ACADEMIC_STATS_PUBLICATIONS_TRENDING_API({userId: user}).apiUrl)))
    .reply(200, mockData.trendingPublications)
    .onGet(new RegExp(escapeRegExp(routes.AUTHORS_SEARCH_API({query: '.*'}).apiUrl)))
    .reply(200, mockData.authorsSearch)
    .onGet(new RegExp(escapeRegExp(routes.SEARCH_KEY_LOOKUP_API({searchQuery: '.*', searchKey: '.*'}).apiUrl)))
    .reply((config) => {
    ...
    .onGet(new RegExp(escapeRegExp(routes.SEARCH_INTERNAL_RECORDS_API({
        searchQuery: '.*',
        pageSize: 5,
        sortBy: 'score',
        sortDirection: 'desc'
    ...
    .onGet(new RegExp(escapeRegExp(routes.SEARCH_EXTERNAL_RECORDS_API({source: 'wos', searchQuery: '.*'}).apiUrl)))
    .reply(200, mockData.externalTitleSearchResultsList)
    .onGet(new RegExp(escapeRegExp(routes.SEARCH_EXTERNAL_RECORDS_API({source: 'scopus', searchQuery: '.*'}).apiUrl)))
    .reply(200, mockData.externalTitleScopusResultsList)
    .onGet(new RegExp(escapeRegExp(routes.SEARCH_EXTERNAL_RECORDS_API({
        source: 'crossref',
    ...
    .onGet(new RegExp(escapeRegExp(routes.GET_PUBLICATION_TYPES_API().apiUrl)))
    .reply(200, mockData.recordsTypeList)
    .onGet(new RegExp(escapeRegExp(routes.VOCABULARIES_API({id: '.*'}).apiUrl))).reply((config) => {
    const vocabId = config.url.substring(config.url.indexOf('/') + 1);
    return [200, mockData.vocabulariesList[vocabId]];
})
    .onGet(new RegExp(escapeRegExp(routes.GET_ACML_QUICK_TEMPLATES_API().apiUrl)))
    .reply(200, mockData.quickTemplates)
    .onGet(new RegExp(escapeRegExp(routes.CURRENT_USER_RECORDS_API({...standardQueryString}).apiUrl)))
    .reply(200, mockData.myRecordsList)
    .onGet(new RegExp(escapeRegExp(routes.POSSIBLE_RECORDS_API({...standardQueryString}).apiUrl)))
