...
export const setup = () => {
    ...
    mock.onGet(routes.CURRENT_ACCOUNT_API().apiUrl)
        ...
        .onGet(
            new RegExp(
                escapeRegExp(
                    routes.THIRD_PARTY_LOOKUP_API_2FIELD({ type: 'incites', field1: '.*', field2: '.*' }).apiUrl,
                ),
            ),
        )
        .reply(200, mockData.lookupToolIncites)
        ...
        .reply(config => {
            ...
            const matchedRecord = mockRecords.find(record => {
                ...
                return (
                    tokens.pop().replace(' ', '') === record.rek_version.replace(' ', '') &&
                    tokens.pop() === record.rek_pid
                );
            });
            if (matchedRecord) {
        ...
        .onGet(new RegExp(escapeRegExp(routes.EXISTING_RECORD_API({ pid: mockData.deletedRecord.rek_pid }).apiUrl)))
        .reply(config => {
            return [410, { data: { ...mockData.deletedRecord } }];
        })
        ...
        .onGet(
            new RegExp(
                escapeRegExp(
                    routes.ADMIN_DASHBOARD_DISPLAY_REPORT_API({ report_type: 2, date_from: '.*', date_to: '.*' })
                        .apiUrl,
                ),
            ),
        )
        ...
        .onGet(
            new RegExp(
                escapeRegExp(
                    routes.ADMIN_DASHBOARD_DISPLAY_REPORT_API({ report_type: 1, date_from: '.*', date_to: '.*' })
                        .apiUrl,
                ),
            ),
        )
        ...
        .onGet(
            new RegExp(
                escapeRegExp(routes.ADMIN_DASHBOARD_DISPLAY_REPORT_API({ report_type: 1, record_id: '.*' }).apiUrl),
            ),
        )
    ...
    mock.onDelete(new RegExp(escapeRegExp(routes.FAVOURITE_SEARCH_LIST_API({ id: '.*' }).apiUrl))).reply(200, {
        data: {},
    });
