...
function setup(record = 'UQ:12345') {
    return rtlRender(<AdminActions record={record} />);
}
...
describe('AdminActions component', () => {
    ...
    it('should handle admin actions menu', () => {
        ...
        const expectedActions = adminActions.map(action => ({
            ...action,
        }));
        ...
        expectedActions.map(action => {
            ...
            expect(global.window.open).toHaveBeenCalledWith(
                `${action.url('UQ:12345')}${!action.isRecordEdit ? '&navigatedFrom=true' : '?navigatedFrom=true'}`,
