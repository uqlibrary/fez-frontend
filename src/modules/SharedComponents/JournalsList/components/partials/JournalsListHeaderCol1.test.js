...
describe('JournalsListHeaderCol1', () => {
    it('should render ', () => {
        ...
        expect(getByTestId('journals-list-header-title-select-all')).not.toHaveAttribute('disabled');
        expect(getByTestId('journals-list-header-title-select-all')).not.toHaveAttribute('checked');
    });
    it('should render without checkboxes when isSelectable = false', () => {
        const { queryByTestId } = setup({ isSelectable: false });
        expect(queryByTestId('journals-list-header-title-select-all')).not.toBeInTheDocument();
    });
    it('should render with select all checked when isAllSelected = true', () => {
        const { getByTestId } = setup({ checked: true });
        expect(getByTestId('journals-list-header-title-select-all')).toHaveAttribute('checked');
    });
    it('should execute given onToggleSelectAll closure', () => {
        ...
        fireEvent.click(getByTestId('journals-list-header-title-select-all'));
