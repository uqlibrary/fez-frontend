...
export const FacetFilterNestedListItemsList = React.memo(
    function FacetFilterNestedListItemsList({
        facetCategory,
        disabled,
        activeFacets,
        handleFacetClick,
        ...
        return facetCategory.facets.map((item, index) => {
            const isActive = isFacetFilterActive(
                activeFacets,
                facetCategory.facetTitle,
                item.key
            );
            return (
...
export default class FacetsFilter extends PureComponent {
    ...
    getNestedListItems = facetCategory => {
        return facetCategory.facets.map((item, index) => {
            const isActive = this.isFacetFilterActive(
                this.state.activeFacets,
                facetCategory.facetTitle,
                item.key
            );
            return (
                <FacetFilterNestedListItem
                    key={index}
                    index={index}
                    onFacetClick={this._handleFacetClick(
                        facetCategory.facetTitle,
                        item.key
                    )}
                    isActive={isActive}
        ...
        Object.keys(rawFacets).forEach(key => {
            ...
            const facetToDisplay = {
                ...
                facets: rawFacet.buckets.map((item, index) => {
                    if (key === 'Display type') {
                        const publicationType = publicationTypes()[
                            rawFacet.buckets[index].key
                        ];
                        return {
                            title:
                        ...
                        const facetValue =
                            rawFacetLookup &&
                            rawFacetLookup.buckets.length > index
    ...
    hasActiveFilters = activeFacets =>
        Object.keys(activeFacets.filters).filter(
    ...
    render() {
        ...
        const dataRangeValueProps = this.state.activeFacets.ranges.hasOwnProperty(
            yearPublishedCategory
        )
            ? {
                        ...
                        return (
                            ...
                            >
                                <FacetFilterNestedListItemsList
                                    ...
                                    isFacetFilterActive={
                                        this.isFacetFilterActive
                                    }
                                />
                            </FacetFilterListItem>
                        );
                    })}
                    {this.props.excludeFacetsList.indexOf(
                        ...
                        <DateRange
                            ...
                            open={
                                this.state.toggledFacets[yearPublishedCategory]
                            }
                        />
                    )}
