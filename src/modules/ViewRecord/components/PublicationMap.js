...
export const GoogleMapViewComponent = withScriptjs(
    withGoogleMap(props => {
...
export const OldPublicationMap = compose(
    lifecycle({
...
const PublicationMap = props => {
    const initialGeoCoords =
        (!!props.coordinates &&
            props.coordinates.split(' ').map(item => ({
                lng: Number(item.split(',')[0]),
                lat: Number(item.split(',')[1]),
            }))) ||
        [];
    ...
    const [bounds] = React.useState(null);
    ...
    const onMapMounted = (bounds, geoCoords) => map => {
        refs.map = map;
        map && bounds && geoCoords.length > 1 && map.fitBounds(bounds);
    };
    ...
    React.useEffect(() => {
        !!props.onChange &&
            props.onChange(
                geoCoords.map(coord => `${trimCoordinates(coord.lng)},${trimCoordinates(coord.lat)}`).join(' '),
            );
    ...
    return (
        <GoogleMapViewComponent
            ...
            onDrawingManagerMounted={onDrawingManagerMounted}
            onSearchBoxMounted={onSearchBoxMounted}
            onPlacesChanged={onPlacesChanged}
            handleMarkerComplete={handleMarkerComplete}
